...
/**Represents a repository of {@link Tag}s to be managed by a {@link BooruClient}.*/
export interface TagStore {
	/**
	 * Obtains multiple {@link Tag}s from this {@link TagStore} based on the provided `names`.
	...
	/**
	 * Obtains a {@link Tag} from this {@link TagStore} based on the provided `name`.
	...
	/**
	 * Stores multiple {@link Tag}s into this {@link TagStore}.
	 * @param tags The tags to store.
	 */
	setMany(tags: Iterable<Tag>): Promise<void>;

	/**
	 * Stores a {@link Tag} into this {@link TagStore}.
	 * @param tag The tag to store.
	 */
	setOne(tag: Tag): Promise<void>;

	/**
	 * Cleans up all {@link Tag}s that are deemed as no longer valid, based on this {@link TagStore}'s criteria.
	 *
